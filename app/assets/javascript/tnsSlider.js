$(".sliderFirst").length && tns({
    container: ".sliderFirst",
    loop: !0,
    startIndex: 1,
    items: 1,
    nav: !1,
    autoplay: 0,
    swipeAngle: !1,
    speed: 400,
    autoplayButtonOutput: !1,
    mouseDrag: !0,
    lazyload: !0,
    gutter: 20,
    controlsContainer: "#sliderFirstControls",
    responsive: {
        0: {
            items: 2
        },
        768: {
            items: 2
        },
        990: {
            items: 6
        }
    }
}), $(".sliderSecond").length && tns({
    container: ".sliderSecond",
    loop: !0,
    startIndex: 1,
    items: 1,
    nav: !1,
    autoplay: 0,
    swipeAngle: !1,
    speed: 400,
    autoplayButtonOutput: !1,
    mouseDrag: !0,
    lazyload: !0,
    gutter: 20,
    controlsContainer: "#sliderSecondControls",
    responsive: {
        0: {
            items: 2
        },
        768: {
            items: 2
        },
        990: {
            items: 6
        }
    }
}), $(".sliderThird").length && tns({
    container: ".sliderThird",
    loop: !0,
    startIndex: 1,
    items: 1,
    nav: !1,
    autoplay: 0,
    swipeAngle: !1,
    speed: 400,
    autoplayButtonOutput: !1,
    mouseDrag: !0,
    lazyload: !0,
    gutter: 20,
    controlsContainer: "#sliderThirdControls",
    responsive: {
        0: {
            items: 2
        },
        768: {
            items: 2
        },
        990: {
            items: 6
        }
    }
}), $(".sliderFourth").length && tns({
    container: ".sliderFourth",
    loop: !0,
    startIndex: 1,
    items: 1,
    nav: !1,
    autoplay: 0,
    swipeAngle: !1,
    speed: 400,
    autoplayButtonOutput: !1,
    mouseDrag: !0,
    lazyload: !0,
    gutter: 20,
    controlsContainer: "#sliderFourthControls",
    responsive: {
        0: {
            items: 2
        },
        768: {
            items: 2
        },
        990: {
            items: 6
        }
    }
}), $(".sliderFifth").length && tns({
    container: ".sliderFifth",
    loop: !0,
    startIndex: 1,
    items: 1,
    nav: !1,
    autoplay: 0,
    swipeAngle: !1,
    speed: 400,
    autoplayButtonOutput: !1,
    mouseDrag: !0,
    lazyload: !0,
    gutter: 20,
    controlsContainer: "#sliderFifthControls",
    responsive: {
        0: {
            items: 2
        },
        768: {
            items: 2
        },
        990: {
            items: 6
        }
    }
}), $(".sliderSixth").length && tns({
    container: ".sliderSixth",
    loop: !0,
    startIndex: 1,
    items: 1,
    nav: !1,
    autoplay: 0,
    swipeAngle: !1,
    speed: 400,
    autoplayButtonOutput: !1,
    mouseDrag: !0,
    lazyload: !0,
    gutter: 20,
    controlsContainer: "#sliderSixthControls",
    responsive: {
        0: {
            items: 2
        },
        768: {
            items: 2
        },
        990: {
            items: 6
        }
    }
}), $(".sliderSeventh").length && tns({
    container: ".sliderSeventh",
    loop: !0,
    startIndex: 1,
    items: 1,
    nav: !1,
    autoplay: 0,
    swipeAngle: !1,
    speed: 400,
    autoplayButtonOutput: !1,
    mouseDrag: !0,
    lazyload: !0,
    gutter: 20,
    controlsContainer: "#sliderSeventhControls",
    responsive: {
        0: {
            items: 2
        },
        768: {
            items: 2
        },
        990: {
            items: 6
        }
    }
}), $(".sliderEighth").length && tns({
    container: ".sliderEighth",
    loop: !0,
    startIndex: 1,
    items: 1,
    nav: !1,
    autoplay: 0,
    swipeAngle: !1,
    speed: 400,
    autoplayButtonOutput: !1,
    mouseDrag: !0,
    lazyload: !0,
    gutter: 20,
    controlsContainer: "#sliderEighthControls",
    responsive: {
        0: {
            items: 2
        },
        768: {
            items: 2
        },
        990: {
            items: 6
        }
    }
}), $(".sliderNinth").length && tns({
    container: ".sliderNinth",
    loop: !0,
    startIndex: 1,
    items: 1,
    nav: !1,
    autoplay: 0,
    swipeAngle: !1,
    speed: 400,
    autoplayButtonOutput: !1,
    mouseDrag: !0,
    lazyload: !0,
    gutter: 20,
    controlsContainer: "#sliderNinthControls",
    responsive: {
        0: {
            items: 2
        },
        768: {
            items: 2
        },
        990: {
            items: 6
        }
    }
}), $(".sliderTestimonial").length && tns({
    container: ".sliderTestimonial",
    loop: !0,
    startIndex: 1,
    items: 1,
    nav: !1,
    autoplay: 0,
    swipeAngle: !1,
    speed: 400,
    autoplayButtonOutput: !1,
    mouseDrag: !0,
    lazyload: !0,
    gutter: 20,
    controlsContainer: "#sliderTestimonialControls",
    responsive: {
        0: {
            items: 2
        },
        768: {
            items: 2
        },
        990: {
            items: 3
        }
    }
}), $(".sliderTestimonialSecond").length && tns({
    container: ".sliderTestimonialSecond",
    loop: !0,
    startIndex: 1,
    items: 1,
    nav: !1,
    autoplay: 0,
    swipeAngle: !1,
    speed: 400,
    autoplayButtonOutput: !1,
    mouseDrag: !0,
    lazyload: !0,
    gutter: 20,
    controlsContainer: "#sliderTestimonialSecondControls",
    responsive: {
        768: {
            items: 1
        },
        990: {
            items: 1
        }
    }
}), $(".product").length && tns({
    container: "#product",
    items: 1,
    startIndex: 1,
    navContainer: "#product-thumbnails",
    navAsThumbnails: !0,
    autoplay: !1,
    autoplayTimeout: 1e3,
    swipeAngle: !1,
    speed: 400,
    controls: !1
}), $(".sliderTestimonialThird").length && tns({
    container: ".sliderTestimonialThird",
    loop: !0,
    startIndex: 1,
    items: 1,
    nav: !1,
    autoplay: !0,
    swipeAngle: !1,
    speed: 400,
    autoplayButtonOutput: !1,
    mouseDrag: !0,
    lazyload: !0,
    gutter: 20,
    controlsContainer: "#sliderTestimonialThirdControls",
    responsive: {
        0: {
            items: 2
        },
        768: {
            items: 2
        },
        990: {
            items: 3
        }
    }
});